...
| `--zone X,Y,W,H` | - | 필기 영역(Write Zone) 지정: 이 사각형 안에서만 시스템/펜 커서를 숨기고 우리 커서 표시. 미지정 시 전체 화면 모드 |
| `--edit` | - | 필기 영역을 편집 모드로 시작 (드래그 이동, 모서리로 크기 조절) |
| `--debug` | - | 커서 숨김 진단용 Win32 메시지 출력 |
...
전체 화면이 아닌 **특정 사각형 영역 안에서만** 시스템/펜 커서를 숨기고
우리 커스텀 커서만 표시할 수 있습니다. 드로잉 앱 캔버스 위에 영역을 놓고 쓰세요.

```bat
python main.py --zone 200,200,1200,800
```

- 영역 안: 커서가 숨겨지고 커스텀 커서가 펜/마우스를 따라갑니다.
- 영역 밖: 평소처럼 정상 동작합니다.
- **Ctrl + Shift + Z**: 편집 모드 토글 — 편집 모드에서 영역을 드래그로 이동하고,
  오른쪽 아래 모서리를 드래그하면 크기를 조절할 수 있습니다.
  (`--edit`로 처음부터 편집 모드로 시작할 수도 있습니다.)
- 필기(마우스/펜 버튼을 누르는) 중에는 잠깐 클릭 통과로 바뀌어 아래 앱이
  실제 입력(펜 압력 포함)을 받고, 버튼을 놓으면 다시 커서를 숨깁니다.
- **진단**: `--debug`를 붙이면 오버레이가 받는 Win32 메시지를 출력합니다.
  `WM 0x0020`(WM_SETCURSOR)이 나오는지 확인해 커서 숨김이 동작하는지
  알 수 있습니다.
